...
def discriminator(x):
    with tf.variable_scope("discriminator"):
        ...
        conv1 = tf.layers.conv2d(inputs=input_layer,filters=64,kernel_size=[3, 3],strides=[1,1], padding="SAME")#32
        bnorm1 = tf.layers.batch_normalization(conv1, axis=3, momentum=0.9, epsilon=1e-5)
        act1 = leaky_relu(bnorm1)

        conv2 = tf.layers.conv2d(inputs=act1,filters=64,kernel_size=[3, 3],strides=[1,1], padding="SAME")#32
        bnorm2 = tf.layers.batch_normalization(conv2, axis=3, momentum=0.9, epsilon=1e-5)
        act2 = leaky_relu(bnorm2)

        conv3 = tf.layers.conv2d(inputs=act2,filters=128, kernel_size=[3, 3], strides=[2,2], padding="SAME")#16
        bnorm3 = tf.layers.batch_normalization(conv3, axis=3, momentum=0.9, epsilon=1e-5)
        act3 = leaky_relu(bnorm3)

        conv4 = tf.layers.conv2d(inputs=act3,filters=128, kernel_size=[3, 3], strides=[2,2], padding="SAME")#8
        bnorm4 = tf.layers.batch_normalization(conv4, axis=3, momentum=0.9, epsilon=1e-5)
        act4 = leaky_relu(bnorm4)   

        conv5 = tf.layers.conv2d(inputs=act4,filters=256, kernel_size=[3, 3], strides=[2,2], padding="SAME")#4
...
def generator(z):
    with tf.name_scope("gen_block"):
        with tf.variable_scope("generator"):
            ...
            deconv1 = tf.layers.conv2d_transpose(act, filters=128, kernel_size=3, strides=2, padding="same")#8
            bnorm1 = tf.layers.batch_normalization(deconv1, axis=3, momentum=0.9, epsilon=1e-5)
            act1 = tf.nn.relu(bnorm1)

            deconv2 = tf.layers.conv2d_transpose(act1, filters=128, kernel_size=3, strides=1, padding="same")#8
            bnorm2 = tf.layers.batch_normalization(deconv2, axis=3, momentum=0.9, epsilon=1e-5)
            act2 = tf.nn.relu(bnorm2)

            deconv3 = tf.layers.conv2d_transpose(act2, filters=64, kernel_size=3, strides=2, padding="same")#16
            bnorm3 = tf.layers.batch_normalization(deconv3, axis=3, momentum=0.9, epsilon=1e-5)
            act3 = tf.nn.relu(bnorm3)

            deconv4 = tf.layers.conv2d_transpose(act3, filters=64, kernel_size=3, strides=1, padding="same")#16
            bnorm4= tf.layers.batch_normalization(deconv4, axis=3, momentum=0.9, epsilon=1e-5)
            act4 = tf.nn.relu(bnorm4)

            deconv5 = tf.layers.conv2d_transpose(act4, filters=3, kernel_size=3, strides=2, padding="same")#32
...
with get_session() as sess:
    with tf.name_scope("original"):
        ...
        tf.summary.image("original", orig_images, max_outputs=6)

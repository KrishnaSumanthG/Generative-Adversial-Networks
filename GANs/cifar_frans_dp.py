...
def generator(z):
    with tf.name_scope("gen_block"):
        with tf.variable_scope("generator"):
            ...
            bnorm1 = tf.layers.batch_normalization(deconv2, axis=3, momentum=0.9, epsilon=1e-5,gamma_initializer=tf.random_normal_initializer(1., 0.02))
            act4 = tf.nn.relu(bnorm1)

            deconv3 = tf.layers.conv2d_transpose(act4, filters=3, kernel_size=5, strides=2, padding="same")
            deconv_final = tf.nn.tanh(deconv3)
...
with get_session() as sess:
    ...
    train_writer = tf.summary.FileWriter( "./frans3", sess.graph)

...
def discriminator(x):
    with tf.variable_scope("discriminator"):
        ...
        conv2 = tf.layers.conv2d(inputs=conv1,filters=128, kernel_size=[5, 5], strides=[2,2], padding="SAME")
        bnorm1 = tf.layers.batch_normalization(conv2, axis=3, momentum=0.9, epsilon=1e-5)
        act1 = leaky_relu(bnorm1)
        
        conv3 = tf.layers.conv2d(inputs=act1,filters=256, kernel_size=[5, 5], strides=[2,2], padding="SAME")
        bnorm2 = tf.layers.batch_normalization(conv3, axis=3, momentum=0.9, epsilon=1e-5)
...
def generator(z):
    with tf.name_scope("gen_block"):
        with tf.variable_scope("generator"):
            ...
            full_2=tf.layers.dense(inputs=z, units=4*4*256)
            ...
            deconv1 = tf.layers.conv2d_transpose(act2, filters=128, kernel_size=5, strides=2, padding="same")
            bnorm = tf.layers.batch_normalization(deconv1, axis=3, momentum=0.9, epsilon=1e-5)
            act3 = tf.nn.relu(bnorm)

            deconv2 = tf.layers.conv2d_transpose(act3, filters=64, kernel_size=5, strides=2, padding="same")
            bnorm1 = tf.layers.batch_normalization(deconv2, axis=3, momentum=0.9, epsilon=1e-5)
...
with get_session() as sess:
    ...
    train_writer = tf.summary.FileWriter( "./deprocess_cifar_summ3", sess.graph)
